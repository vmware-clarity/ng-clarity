...
/**
 * A styles-only file: the template is raw `clr-form` markup with no Clarity component and no story
 * wrapper to base the args on, so the args are declared explicitly.
 */
type FormsInputStatesStylesArgs = {
            ...
            @if (isSuccess || isError) {
              <div class="clr-subtext-wrapper" [ngClass]="{ success: isSuccess, error: isError }">
                <cds-icon
                  ...
                  [shape]="isSuccess ? 'success-standard' : 'error-standard'"
                ></cds-icon>
...
type Story = StoryObj<FormsInputStatesStylesArgs>;
