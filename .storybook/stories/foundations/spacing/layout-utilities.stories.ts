...
/** The four stories are pure markup demos of the `cds-layout` utilities; none of them takes an arg. */
type LayoutUtilitiesArgs = Record<never, never>;

/**
 * The `<style>` block that three of the four templates interpolated as `${PLACEHOLDER_STYLES}`, followed by
 * the one that opened the Container Widths template. Both rule sets are copied verbatim.
 *
 * `withStyles()` is a meta decorator, so it reaches all four stories rather than three and one. That is safe
 * here because the two rule sets are class-name disjoint -- `.cds-placeholder`/`.cds-demo*` are used only by
 * the first three stories and `.container-demo*` only by the fourth -- so neither half matches anything in
 * the stories that did not have it before.
 */
const LAYOUT_UTILITIES_STYLES = `
  .cds-placeholder {
    display: flex;
...
const meta: Meta<LayoutUtilitiesArgs> = {
  ...
  decorators: [
    ...
    withStyles(LAYOUT_UTILITIES_STYLES),
  ],
  render: () => ({
    template: `
...
type Story = StoryObj<LayoutUtilitiesArgs>;
