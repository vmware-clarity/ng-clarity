 ...
 *
 * A story file whose base name repeats its directory name (`accordion/accordion.stories.ts`) is that
 ...
 *
 * The title depends on the file's own path (and `overrides`) only, never on which other files
 * exist, so adding or removing a sibling story file can never retitle an untouched one.
 */
function canonicalTitle(located, overrides) {
  const fileOverride = overrides[located.relativePath];

  if (typeof fileOverride === 'string') {
    return fileOverride;
  }

  const directoryOverride = overrides[located.relativeDirectory];
  const directoryTitle =
    typeof directoryOverride === 'string'
      ? directoryOverride
      : located.relativeDirectory.split('/').map(titleCase).join('/');

  if (located.base === located.directoryName) {
...
// Type-only wrappers around the meta object: `{…} satisfies Meta<X>`, `{…} as Meta<X>`, `<Meta<X>>{…}`.
const TYPE_WRAPPERS = new Set(['TSSatisfiesExpression', 'TSAsExpression', 'TSTypeAssertion']);

/**
 * The story meta object: `const meta = {…}` (the canon) or a bare `export default {…}`, either of
 * them optionally wrapped in `satisfies Meta<X>`, `as Meta<X>` or `<Meta<X>>`.
 */
function isMetaObject(node) {
  ...
  while (
    ...
    TYPE_WRAPPERS.has(expression.parent.type) &&
  ...
  if (parent.type === 'ExportDefaultDeclaration') {
    return parent.declaration === expression;
  }
...
/**
 * Builds the rule around a title overrides map. The exported rule uses the real
 * `storybook-title-overrides.js`; the factory exists so the tests can inject their own map.
 */
function createRule(overrides) {
  /**
   * Story root -> `{ signature, byTitle }`, where `byTitle` maps every derived title to the files
   * deriving it. A title depends only on its file's path and `overrides`, so the map is valid for as
   * long as the set of story files is unchanged; `signature` is that set (the walk's sorted paths).
   * The tree is walked again for every linted file (under a millisecond for ~125 files) and the map
   * rebuilt only when the signature differs, so a long-lived ESLint server (an editor integration)
   * sees story files being added, removed or moved. `overrides` itself is read once, at load.
   */
  const siblingTitleCache = new Map();

  /** Files that resolve to `title` other than `self`; both cross-file checks below need it. */
  function filesSharingTitle(storiesRoot, title, self) {
    const files = allStoryFiles(storiesRoot);
    const signature = files.join('\n');
    let cached = siblingTitleCache.get(storiesRoot);
    ...
    if (cached === undefined || cached.signature !== signature) {
      ...
      for (const file of files) {
        ...
        const fileTitle = canonicalTitle(located, overrides);
      ...
      cached = { signature: signature, byTitle: byTitle };
      siblingTitleCache.set(storiesRoot, cached);
    }

    return (cached.byTitle.get(title) ?? []).filter(file => file !== self);
  }
  ...
  return {
    ...
    create: context => {
      ...
      const expected = canonicalTitle(located, overrides);
      ...
      return {
        ...
        'Program:exit': node => {
          ...
          // name they have in common yields a duplicate story id, which Storybook refuses to index
          // (the build fails). Both halves of that hazard are reported here: the collision itself,
          // and — belt and braces — the colliding export names.
          if (shared.length === 0) {
            return;
          }
            ...
            if (clashing.length > 0) {
              context.report({
                ...
                message: `Story export ${clashing.join(', ')} is declared in both this file and ${relative([
                  ...
                  expected
                )}. Duplicate export names under one title produce duplicate story ids, which fail the Storybook build.`,
              });
            }
          }
        },
...
module.exports = createRule(TITLE_OVERRIDES);
// Not part of the rule: lets the tests build the rule around their own overrides map.
