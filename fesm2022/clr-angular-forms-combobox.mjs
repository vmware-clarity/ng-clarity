...
import { ArrowKeyDirection, Keys, customFocusableItemProvider, uniqueIdFactory, ClrLoadingState, IF_ACTIVE_ID, LoadingListener, hasRequiredValidator, publishElementContext, IF_ACTIVE_ID_PROVIDER, FOCUS_SERVICE_PROVIDER, ClrConditionalModule, ClrKeyFocusModule } from '@clr/angular/utils';
...
class ClrCombobox extends WrappedFormControl {
     ...
     */
    publishContext(host) {
        const describe = snapshotOptions => {
            ...
            const state = { multiSelect: this.multiSelect };
    ...
    optionLabel(option) {
        ...
        option.elRef.nativeElement.childNodes.forEach(node => {
            ...
            const isVisibleElement = node.nodeType === Node.ELEMENT_NODE && !node.classList.contains('clr-sr-only');
