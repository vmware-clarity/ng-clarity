...
| card-collapsible           | Marks a card as collapsible                                                                                                           |
| card-collapsed             | Collapsed state of a `.card-collapsible`; toggling it animates the `.card-collapsible-content` with CSS only                          |
| card-header-toggle         | The chevron button inside `.card-header` of a collapsible card; its `aria-expanded` attribute drives the chevron rotation             |
| card-header-icon           | The chevron icon within `.card-header-toggle`                                                                                         |
| card-collapsible-content   | Wraps the collapsible content of a card (`role="region"`); a `.card-footer` placed after it stays visible while the card is collapsed |
| card-collapsible-inner     | The single child of `.card-collapsible-content` that clips the content during the transition                                          |
...
| clr-card-header-content    | Wraps the content projected into `<clr-card-header>`; labels the collapsible region and describes the toggle button                   |
...
| clr-card-img               | Host class on the `<clr-card-img>` Angular component, alongside `card-img`                                                            |
