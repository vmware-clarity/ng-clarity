...
export class DropdownFocusHandler implements OnDestroy, FocusableItem {
  ...
  set trigger(el: HTMLElement) {
    ...
    if (this.parent) {
      this._unlistenFuncs.push(
        this.renderer.listen(el, 'keydown.arrowright', event => this.stateService.toggleWithEvent(event))
      );
    } else {
      this._unlistenFuncs.push(
        this.renderer.listen(el, 'keydown.arrowup', event => this.stateService.toggleWithEvent(event))
      );
      this._unlistenFuncs.push(
        this.renderer.listen(el, 'keydown.arrowdown', event => this.stateService.toggleWithEvent(event))
      );
  ...
  set container(el: HTMLElement) {
    ...
    this._unlistenFuncs.push(
      this.renderer.listen(el, 'keydown.tab', event => this.stateService.toggleWithEvent(event))
    );
    ...
    if (this.parent) {
      ...
      this._unlistenFuncs.push(
        this.renderer.listen(el, 'keydown.escape', event => this.stateService.toggleWithEvent(event))
      );
  ...
  moveToFirstItemWhenOpen() {
    const subscription = this.stateService.openChange.subscribe(open => {
      if (open && this.stateService.originalEvent) {
  ...
  private openAndGetChildren() {
    return wrapObservable(this.children, () => (this.stateService.open = true));
  }

  private closeAndGetThis() {
    return wrapObservable(of(this), () => (this.stateService.open = false));
  }
}
