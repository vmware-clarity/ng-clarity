...
async function main() {
  const compiledContentPath = './projects/clarity.design/src/compiled-content';
...
async function compileCmsNav() {
  ...
  for (const navFilePath of glob.sync('cms-content/nav/*.md')) {
...
async function compileCmsPages() {
  ...
  for (const pageFilePath of glob.sync('cms-content/pages/*.md')) {
...
async function compileStyleDocs() {
  const styleDocs = fs.readFileSync('./dist/clr-ui/STYLES.md').toString();
...
async function compileStackBlitzExampleTemplate() {
  ...
  // add files
  for (const filePath of glob.sync('projects/clarity.design/stackblitz-example-template/**/*.*')) {
    const [,,, ...relativeFilePath] = filePath.split(path.sep);
    files[relativeFilePath.join('/')] = fs.readFileSync(filePath).toString();
  }

  // set dependency versions in package.json files
  const repoPackageManifest = require('../../../package.json');
  ...
  for (const dependency of Object.keys(templatePackageManifest.dependencies)) {
    if (repoPackageManifest.devDependencies[dependency]) {
      templatePackageManifest.dependencies[dependency] = repoPackageManifest.devDependencies[dependency];
    }
  }

  if (templatePackageManifest.devDependencies) {
    for (const devDependency of Object.keys(templatePackageManifest.devDependencies)) {
      if (repoPackageManifest.devDependencies[devDependency]) {
        templatePackageManifest.devDependencies[devDependency] = repoPackageManifest.devDependencies[devDependency];
      }
    }
  }
