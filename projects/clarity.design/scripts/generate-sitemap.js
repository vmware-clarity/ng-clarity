...
async function main() {
  ...
  fs.writeFileSync(path.join('dist', 'website', 'pages.json'), JSON.stringify(pages, undefined, 2));

  const sitemap = await generateSitemap(pages);
  fs.writeFileSync(path.join('dist', 'website', 'sitemap.xml'), sitemap);
}
...
function startServer(port) {
  return new Promise(resolve => {
    ...
    app.use(express.static(path.resolve('./dist/website')));
