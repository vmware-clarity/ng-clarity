...
function getFilteredChanges(filter) {
  return getChanges()
    ...
    .filter(file => !filter || minimatch.match([file.name], filter, { dot: true, matchBase: true }).length > 0)
    .map(file => file.name);
}
...
function getFile(entry) {
  ...
  const status = data[0];
  let name = ['R', 'RM'].includes(status) ? data[3] : data[1];
  if (name) {
    name = name.replace('clarity.design', '.');
  }
  return { status, name };
}
