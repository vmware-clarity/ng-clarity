...
async function main() {
  ...
  writeJson('api-docs.json', await compileApiDocs());
...
/**
 * Merges the API report files generated by `npm run website-api:update`
 * (see scripts/website-api-docs.js) into a single map keyed by class name.
 */
async function compileApiDocs() {
  const apiDocs = {};

  for (const reportPath of glob.sync(path.join(PROJECT_ROOT, 'content/api-docs/**/*.json'), {
    windowsPathsNoEscape: true,
  })) {
    const report = JSON.parse(fs.readFileSync(reportPath).toString());

    for (const component of report.components) {
      if (apiDocs[component.name]) {
        throw new Error(`Duplicate API docs entry for ${component.name} in ${reportPath}`);
      }
      apiDocs[component.name] = { ...component, entryPoint: report.entryPoint };
    }
  }
