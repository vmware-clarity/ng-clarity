...
export class DatagridVirtualScrollDemo implements OnInit, OnDestroy {
  ...
  rowSize = 'default';
  selected: User[] = [];
  readonly datagrid = viewChild<ClrDatagrid>('datagrid');
  ...
  constructor(
    ...
    inventory.size = this.currentPageSize * 3;
    inventory.latency = 500;
    inventory.lazyLoadUsers(inventory.size);
    this.users = inventory.getAllUsersSubject().asObservable();
