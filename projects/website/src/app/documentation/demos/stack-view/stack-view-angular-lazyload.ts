...
import { ClrModalModule, ClrSpinnerModule, ClrStackBlock, ClrStackViewModule } from '@clr/angular';
...
export class StackViewAngularLazyloadDemo extends StackViewNgDemo {
  readonly lazyBlock = viewChild<ClrStackBlock>('lazyBlock');

  example = EXAMPLE;
  code = CODE;

  resetChildren(): void {
    const lazyBlock = this.lazyBlock();
    if (lazyBlock) {
      lazyBlock.expanded = false;
    }
