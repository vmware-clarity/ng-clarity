...
export class WizardJumpToDemo {
  ...
  readonly pageThree = viewChild<ClrWizardPage>('pageThree');
  readonly pageFive = viewChild<ClrWizardPage>('pageFive');
  ...
  jumpToThree(): void {
    const pageThree = this.pageThree();
    if (pageThree) {
      this.jumpTo(pageThree);
    }
  }

  jumpToFive(): void {
    const pageFive = this.pageFive();
    if (pageFive) {
      this.jumpTo(pageFive);
    }
  }
}
