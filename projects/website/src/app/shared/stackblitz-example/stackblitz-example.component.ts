...
export class StackblitzExampleComponent {
  ...
  readonly additionalFiles = input<
    | {
  ...
  readonly showComponentTemplate = input(true);

  protected copied = false;

  private readonly activeCodeSnippet = viewChild(CodeSnippetComponent);
  ...
  protected openInStackBlitz() {
    ...
    const additionalFiles = this.additionalFiles();
    if (additionalFiles) {
      for (const [relativeFilePath, fileContents] of Object.entries(additionalFiles)) {
