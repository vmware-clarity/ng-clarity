...
export class ThemeBuilderComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  get primaryContrastOnWhite(): number {
    return contrastRatio(this.current.primary, [0, 0, 100]);
  }

  get primaryContrastOnBlack(): number {
    return contrastRatio(this.current.primary, [0, 0, 0]);
  }
  ...
  get textContrastOnContainer(): number {
    return contrastRatio(this.current.text, this.current.containerBg);
  }
  ...
  setCurrentColor(key: keyof ThemeColors, hex: string): void {
    if (/^#[0-9a-fA-F]{6}$/.test(hex)) {
      this.colors[this.activeTheme][key] = hexToHsl(hex);
      this.onColorChange();
    }
  }
  ...
  applyPreviewStyles(): void {
    ...
    const vars: Record<string, string> = {
      '--cds-alias-status-info': hsl(c.primary),
      '--cds-alias-status-info-tint': ev(d.primaryTint),
      '--cds-alias-status-info-shade': ev(d.primaryShade),
      '--cds-alias-object-interaction-background-highlight': hsl(c.primary),
      ...
      '--cds-alias-status-warning': hsl(c.warning),
      ...
      '--cds-alias-status-danger': hsl(c.danger),
      ...
      '--cds-alias-object-app-background': hsl(c.appBg),
      '--cds-alias-object-container-background': hsl(c.containerBg),
      '--cds-alias-object-overlay-background': hsl(c.containerBg),
      '--cds-alias-typography-color-500': hsl(c.text),
    };
    for (const [prop, val] of Object.entries(vars)) {
      el.style.setProperty(prop, val);
    }
    el.style.backgroundColor = hsl(c.appBg);
    el.style.color = hsl(c.text);
  }
